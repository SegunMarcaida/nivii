"""Run golden coverage-v2 NL2SQL evaluation and enforce pass-rate gate."""
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Run golden coverage-v2 NL2SQL gate")
    parser.add_argument(
        "--fixture",
        default="api/tests/fixtures/golden_queries_coverage_v2.json",
        help="Path to golden fixture JSON",
    )
    parser.add_argument("--threshold", type=float, default=0.99, help="Minimum valid rate")
    ...
    print(f"golden_eval_v2: valid={result['valid']}/{result['total']} ({rate_pct:.2f}%)")
    ...
    if result["valid_rate"] < args.threshold:
        print(f"Gate failed: valid_rate {result['valid_rate']:.4f} < threshold {args.threshold:.4f}")
        return 1

    print(f"Gate passed: valid_rate {result['valid_rate']:.4f} >= threshold {args.threshold:.4f}")
    return 0
